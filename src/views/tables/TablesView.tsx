      ...
      <div class="main-area">
        <div class="header-bar">
          <div class="header-bar-title">
            <span aria-hidden="true" class="material-symbols-rounded">table_bar</span>
            <h2>Table Management</h2>
          </div>
          <button id="add-table-btn" class="btn btn-primary btn-sm">
            <span aria-hidden="true" class="material-symbols-rounded" style="font-size:16px;">add</span> Add Table
          </button>
        </div>
      ...
      <div id="table-modal" class="modal-overlay" style="display:none;">
        <div class="modal" style="max-width:360px;">
          <div class="modal-header">
            <h3>Add Table</h3>
            <button class="btn-icon" id="tbl-close-icon"><span aria-hidden="true" class="material-symbols-rounded">close</span></button>
          </div>
    ...
    grid.innerHTML = `<div class="content-grid" style="grid-template-columns: repeat(auto-fill, minmax(130px, 1fr));">${tables.map(t => {
      ...
      return `
        <div class="table-card card" data-id="${escapeHtml(String(t.id ?? ''))}" style="border: 1.5px solid ${cfg.border}; background: ${cfg.bg}; text-align: center; cursor: pointer; padding: 18px 14px;">
          ...
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size:24px; color:${cfg.color}; display:block; margin-bottom:6px; filter: drop-shadow(0 0 6px ${cfg.border});">${cfg.icon}</span>
