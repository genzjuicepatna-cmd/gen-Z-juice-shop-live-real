// @ts-nocheck
/*
 * supabaseClient is imported lazily, inside sendToCloud.
 *
 * main.ts imports this module eagerly — it has to, because the constructor is
 * what installs the global error handlers, and an error before that point goes
 * unrecorded. But a static import here dragged the whole Supabase SDK into the
 * customer's critical path for the sake of a call that only runs when there is
 * something to upload. The storefront now downloads it only if it needs it.
 */
...
class TelemetryService {
  constructor() {
    ...
    this.storeId = localStorage.getItem('store_id') || STORE_ID;
