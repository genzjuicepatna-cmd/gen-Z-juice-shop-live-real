...
const CUSTOMER_PREFERENCE_OPTIONS = [
  { key: 'less_sugar', label: 'Less sugar', icon: 'remove' },
  { key: 'no_ice', label: 'No ice', icon: 'stop_circle' },
  { key: 'call_before_delivery', label: 'Call before delivery', icon: 'call' },
  { key: 'no_straw', label: 'Skip the straw', icon: 'eco' },
        ...
        <nav className="store-nav-links" aria-label="Customer navigation">
          ...
          {loggedInCustomer ? (
            <button type="button" className="store-rewards-link store-nav-text-button" onClick={() => onNavigate('account')}>
              <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '16px' }}>account_circle</span>
              Account
            </button>
          ) : (
            <button onClick={onSignIn} className="store-login-btn" type="button">
              <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '14px' }}>login</span>
              Sign In
            </button>
          )}
          {loggedInCustomer && (
            <button onClick={onLogout} className="store-logout-btn" type="button" title="Sign Out">
              <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '18px' }}>logout</span>
            </button>
          )}
        </nav>
      </header>
...
export function CustomerApp({ app }) {
  ...
  const loadData = async () => {
    ...
    const resolvedSettings = {
      name: storeId !== STORE_ID ? storeId.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : (name || BRAND.name),
      tagline: storeId !== STORE_ID ? 'Fresh & Delicious Online Store' : (tagline || BRAND.tagline),
      phone: phone || '',
      address: address || ''
    };
  ...
  const handleAddToCartFromDrawer = ({ item, qty, sugar, modifiers, customNotes, price }) => {
    // These notes are what the counter reads off the ticket, so they say the
    // thing plainly. "Spicy: Mild" on a glass of orange juice was noise at
    // best and a misread order at worst.
    let consolidatedNotes = `Sugar: ${sugar}`;
    if (modifiers.length > 0) consolidatedNotes += ` | Add: ${modifiers.join(', ')}`;
  ...
  // Dynamic values
  // The kicker is owner-editable (Branding writes STOREFRONT_SETTING_KEYS
  // .heroKicker) and the pre-rendered shell already renders it. This used to
  // hardcode 'Order online from home', so editing the field changed the
  // crawlable HTML but not the running app, and the two disagreed on the
  // handoff.
  const modeStr = detectedTable ? `Table ${detectedTable.number}` : storefrontCopy.heroKicker;
  const displayAddress = storeSettings.address || addressLine();
    ...
    <div className={`storefront-root ${accessibilityClass}`}>
      ...
      {state === 'menu' && (
        <div className="storefront-shell">
          ...
          {customerPage === 'home' && <>
          {/* Hero Section */}
          <section className="store-hero" aria-label={`${BRAND.name} storefront`}>
          ...
          <section className="store-service-band" id="order-options" aria-label="Ordering options">
            <button className={`store-service-option ${orderType === 'delivery' ? 'is-active' : ''}`} aria-pressed={orderType === 'delivery'} onClick={() => setOrderType('delivery')} type="button">
              ...
              <small>Brought to your door</small>
            </button>
              ...
              {menuLoading && featuredItems.length === 0
                ...
                <article key={item.id} className="store-featured-item">
                  <button
                    ...
                    type="button"
                  >
                    {/* itemArt, not categoryArt — the rail was showing the same
                        orange cup for Orange Juice, Anar Juice and Mix Juice. */}
                    <img src={menuItemImageSource(item) || itemArt(item.name, categories.find(c => c.id === item.categoryId)?.name)} alt="" width="640" height="420" loading="lazy" decoding="async" />
                    <span>{item.name}</span>
          ...
          <section className="store-info-band" aria-label="Payment and ordering details">
            ...
            <div>
              ...
              <p>Orders move from the counter to assignment, out-for-delivery, and delivered.</p>
            </div>
          </section>

          <footer className="customer-footer">
            <div><strong>{BRAND.name}</strong><p>{storefrontCopy.footerCopy}</p></div>
            ...
            {/* Was a hardcoded string here while the static shell rendered
                STOREFRONT_DEFAULTS.footerNote — so the pre-boot page and the
                booted SPA said different things, and the content layer was
                being ignored by the half people actually read. */}
            <small>{STOREFRONT_DEFAULTS.footerNote}</small>
          </footer>
          </>}
        ...
        <div className="store-checkout-shell">
          <header className="store-subheader">
            ...
            <a href="#/self-order" className="store-subheader-brand">
              <img src="/assets/store-logo.svg" className="store-brand-mark" style={{ width: '24px', height: '24px', borderRadius: '4px', objectFit: 'contain', background: '#fff', marginRight: '4px' }} alt="" />
              {BRAND.name}
            </a>
            <h2>Checkout</h2>
          </header>
              ...
              <section className="store-checkout-panel" style={{ marginBottom: '20px' }}>
                ...
                <div className="store-option-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                  <button className={`store-choice-btn ${orderType === 'delivery' ? 'is-active' : ''}`} type="button" onClick={() => setOrderType('delivery')}>
                    <span aria-hidden="true" className="material-symbols-rounded">local_shipping</span>
                    <span>Delivery</span>
                  </button>
                  <button className={`store-choice-btn ${orderType === 'takeaway' ? 'is-active' : ''}`} type="button" onClick={() => setOrderType('takeaway')}>
                    <span aria-hidden="true" className="material-symbols-rounded">shopping_bag</span>
                    <span>Pickup</span>
                  </button>
                  <button className={`store-choice-btn ${orderType === 'dinein' ? 'is-active' : ''}`} type="button" onClick={() => setOrderType('dinein')}>
                    <span aria-hidden="true" className="material-symbols-rounded">table_restaurant</span>
                    <span>Dine-in</span>
                  </button>
                </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                      ...
                      >
                        {locating ? (
                          ...
                          <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '14px' }}>my_location</span>
                        )}
                      ...
                      {addressSuggestions.length > 0 && (
                        ...
                        >
                          {addressSuggestions.map(s => (
                            ...
                            >
                              <span aria-hidden="true" className="material-symbols-rounded" style={{ color: 'var(--color-primary)', fontSize: '18px', flexShrink: 0 }}>location_on</span>
            ...
            <section className="store-checkout-panel" style={{ marginBottom: '20px' }}>
              ...
              <div className="store-option-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <button className={`store-choice-btn ${selectedPaymentMethod === 'upi' ? 'is-active' : ''}`} type="button" onClick={() => setSelectedPaymentMethod('upi')}>
                  <span aria-hidden="true" className="material-symbols-rounded">qr_code_2</span>
                  <span>UPI QR</span>
