...
export class DevConsoleView {
  ...
  async render() {
    ...
    const tabsHTML = TABS.map(t => `
      <button class="dev-tab ${this.activeTab === t.id ? 'active' : ''}" data-tab="${t.id}">
        <span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;">${t.icon}</span>
        <span>${t.label}</span>
      </button>
    `).join('');
      ...
      <div style="flex:1;display:flex;flex-direction:column;height:100%;overflow:hidden;background:var(--bg-primary);">
        ...
        <div style="display:flex;align-items:center;gap:12px;padding:16px 24px;background:var(--glass-bg);backdrop-filter:blur(20px);border-bottom:1px solid var(--border-glass);z-index:10;">
          <div style="width:40px;height:40px;border-radius:12px;background:rgba(var(--color-success-rgb),0.1);border:1px solid rgba(var(--color-success-rgb),0.25);display:flex;align-items:center;justify-content:center;">
            <span aria-hidden="true" class="material-symbols-rounded" style="font-size:22px;color:var(--color-success-on-surface);filter:drop-shadow(0 0 6px rgba(var(--color-success-rgb),0.4));">terminal</span>
          </div>
          <div>
        ...
        <!-- Tabs -->
        <div style="display:flex;gap:4px;padding:12px 24px;background:var(--bg-secondary);border-bottom:1px solid var(--border-color);overflow-x:auto;">
          ${tabsHTML}
        </div>
  ...
  async renderSystemInfo(el) {
    ...
    el.innerHTML = `
      <div class="dev-card">
        <h3><span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color:var(--color-success-on-surface);">info</span> Application</h3>
      ...
      <div class="dev-card">
        <h3><span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color:var(--nextgenos-purple-on-surface);">database</span> Database</h3>
      ...
      <div class="dev-card">
        <h3><span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color: var(--color-warning-on-surface);">cloud</span> Cloud Connection</h3>
  ...
  async renderSettingsEditor(el) {
    ...
    el.innerHTML = `
      <div class="dev-card">
        <h3>
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color: var(--color-warning-on-surface);">settings</span>
  ...
  async renderAuditBrowser(el) {
    ...
    el.innerHTML = `
      <div class="dev-card">
        <h3><span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color: var(--color-danger-on-surface);">shield</span> Audit Events (Cloud)</h3>
  ...
  async renderFeatureFlags(el) {
    ...
    el.innerHTML = `
      <div class="dev-card">
        <h3><span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color:var(--nextgenos-purple-on-surface);">flag</span> Feature Flags</h3>
  ...
  async renderAIConfig(el) {
    el.innerHTML = `
      <div class="dev-card">
        <h3><span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color: var(--color-warning-on-surface);">smart_toy</span> AI Provider Configuration</h3>
        ...
        <div style="margin-bottom:20px;">
          <div style="font-weight:700;color:var(--nextgenos-purple-on-surface);font-size:0.8rem;margin-bottom:8px;">🧠 Tier 3 — Lightning AI (Complex Analytics)</div>
          <div class="dev-row"><span class="dev-key">Status</span><span>Protected Edge Function</span></div>
        </div>

        <div style="display:flex;gap:8px;">
          <button class="dev-btn" id="test-groq-btn" style="border-color: var(--color-success);">Test Groq</button>
          <button class="dev-btn" id="test-lightning-btn" style="border-color:rgba(var(--nextgenos-purple-rgb),0.25);color:var(--nextgenos-purple-on-surface);">Test Lightning</button>
        </div>
      </div>

      <div class="dev-card">
        <h3><span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color:var(--text-muted);">computer</span> Tier 1 — Codebase AI (Local)</h3>
