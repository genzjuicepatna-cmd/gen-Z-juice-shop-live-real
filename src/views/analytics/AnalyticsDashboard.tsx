...
export class AnalyticsDashboard {
      ...
      <div style="flex:1;display:flex;flex-direction:column;height:100%;overflow:hidden;background:var(--bg-primary);">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 24px;background:var(--glass-bg);backdrop-filter:blur(20px);border-bottom:1px solid var(--border-glass);z-index:10;flex-wrap:wrap;gap:12px;">
          <div style="display:flex;align-items:center;gap:10px;">
            <span aria-hidden="true" class="material-symbols-rounded" style="color: var(--color-primary-on-surface);font-size:24px;filter:drop-shadow(0 0 8px rgba(var(--color-primary-rgb),0.45));">analytics</span>
        ...
        <div style="flex:1;overflow-y:auto;padding:20px 24px;display:flex;flex-direction:column;gap:20px;max-width:1100px;margin:0 auto;width:100%;">
          ...
          <div class="card card-glass" style="padding:20px;background:rgba(255,255,255,0.01);border:1px solid var(--border-glass);border-radius:16px;display:flex;flex-direction:column;gap:16px;">
            <div style="font-family:'Plus Jakarta Sans',sans-serif;font-size:var(--text-sm);font-weight:700;color:var(--text-primary);display:flex;align-items:center;gap:8px;">
              <span aria-hidden="true" class="material-symbols-rounded" style="color: var(--color-primary-on-surface);font-size:20px;">table_view</span>
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;">
              <button class="btn btn-secondary report-download-btn" data-type="daily" style="
                ...
                transition:all 0.2s;
              ">
                <span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color:var(--color-success-on-surface);">today</span>
              ...
              <button class="btn btn-secondary report-download-btn" data-type="weekly" style="
                ...
                transition:all 0.2s;
              ">
                <span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color: var(--color-primary-on-surface);">date_range</span>
              ...
              <button class="btn btn-secondary report-download-btn" data-type="monthly" style="
                ...
                transition:all 0.2s;
              ">
                <span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color: var(--color-info-on-surface);">calendar_month</span>
  ...
  bindEvents() {
    ...
    this.container.querySelectorAll('.report-download-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        btn.disabled = true;
        btn.innerHTML = `<span aria-hidden="true" class="material-symbols-rounded animate-spin" style="font-size:18px;">sync</span> Generating...`;
        ...
        try {
          ...
          if (blob && filename) {
            ...
            if (isDriveConnected && autoUpload) {
              if (statusEl) {
                statusEl.innerHTML = `<span aria-hidden="true" class="material-symbols-rounded animate-spin" style="font-size:12px;color: var(--color-info-on-surface);">sync</span> Uploading backup to Google Drive...`;
              }
              ...
              try {
                ...
                if (uploadResult.success) {
                  ...
                  if (statusEl) {
                    statusEl.innerHTML = `✅ Report backed up to Google Drive folder: <strong>${escapeHtml(BRAND.name)} Reports</strong>`;
                  }
                }
              } catch (uploadErr) {
  ...
  renderKPIs(stats, growth) {
    ...
    const kpis = [
      ...
      { label: "GROWTH RATE", value: `${growth > 0 ? '+' : ''}${growth.toFixed(1)}%`, color: 'var(--nextgenos-purple-on-surface)', glow: 'rgba(108,92,231,0.25)' },
    ];
  ...
  renderOrderTypes(types, totalOrders) {
    ...
    const colors = { takeaway: '#FF6B35', dinein: 'var(--color-success-on-surface)', delivery: 'var(--color-info-on-surface)' };
