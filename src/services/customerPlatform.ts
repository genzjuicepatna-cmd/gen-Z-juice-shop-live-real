...
export const FALLBACK_OFFERS = [
  /*
   * The shown-when-nothing-is-configured offers. These were "Wok & sip" —
   * pair selected noodles with a chilled drink — and "Table feast", a spread
   * of starters, mains and rice. Both were the previous tenant's, and both
   * rendered on the customer Offers page whenever the server had no live
   * offers to send, which is every install until someone configures one.
   *
   * They are the three combos printed on the shop's own board now.
   *
   * The board price stays in the description rather than in `displayValue`.
   * That field is the prominent number on the card, and a hard price there
   * reads as a guarantee — but the server owns eligibility and final price at
   * checkout, which is the whole point of the copy on this page.
   * tests/customer_platform.test.ts asserts that language survives.
   */
  {
    id: 'local-power-boost',
    code: 'POWERBOOST',
    title: 'Power Boost',
    label: 'Combo favourite',
    description: 'Any juice plus any shake — ₹120 on the board.',
    displayValue: 'Server validates at checkout',
    icon: 'bolt',
    source: 'local',
  },
  {
    id: 'local-fruit-fusion',
    code: 'FRUITFUSION',
    title: 'Fruit Fusion',
    label: 'Juice and a bite',
    description: 'Any juice with a fruit chaat on the side — ₹100 on the board.',
    displayValue: 'Server validates at checkout',
    icon: 'local_offer',
    source: 'local',
  },
  {
    ...
    title: 'Cool Duo',
    label: 'For two',
    description: 'Any shake plus a cold coffee — ₹110 on the board.',
    displayValue: 'Server validates at checkout',
    icon: 'groups',
    source: 'local',
  },
  {
    ...
    label: BRAND.rewardsName,
    description: 'Sign in before checkout and grow your rewards balance with every order.',
...
export async function syncCustomerFavorite({ customer, item }) {
  ...
  try {
    ...
    const { error } = await supabase.from('customer_favorites').upsert({
      ...
      store_id: localStorage.getItem('store_id') || STORE_ID,
