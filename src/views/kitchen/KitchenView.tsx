      ...
      <div style="flex: 1; display: flex; flex-direction: column; height: 100%; overflow: hidden; background: var(--bg-primary);">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 16px 24px; background: var(--glass-bg); backdrop-filter: blur(20px); border-bottom: 1px solid var(--border-glass); z-index: 10;">
          <div style="display: flex; align-items: center; gap: 10px;">
            <span aria-hidden="true" class="material-symbols-rounded" style="color: var(--color-primary-on-surface); font-size: 24px; filter: drop-shadow(0 0 8px rgba(var(--color-primary-rgb), 0.45));">restaurant</span>
          ...
          <button class="btn btn-secondary btn-sm" id="btn-refresh-kds" style="
            ...
            font-weight: 700;
          ">
            <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 18px;">refresh</span>
            Refresh Board
          </button>
        </div>
    ...
    if (element.children.length === 0) {
      element.innerHTML = `
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; color: var(--text-muted); padding: 40px 0;">
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 32px; margin-bottom: 6px;">inbox</span>
  ...
  createOrderCard(order) {
    ...
    if (order.status !== 'ready') {
      if (elapsedMinutes >= 15) {
        ageColor = 'var(--color-danger-on-surface)';
        cardStyle = `
          padding: 20px;
        ...
        ageColor = 'var(--color-warning-on-surface)';
        cardStyle = `
          padding: 20px;
        ...
        <div style="display: flex; align-items: flex-start; justify-content: space-between; font-size: var(--text-sm); line-height: 1.5; padding: 4px 0;">
          <div style="flex: 1; min-width: 0; padding-right: 12px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="font-family: 'Plus Jakarta Sans', sans-serif; font-weight: 800; color: var(--color-primary-on-surface); font-size: 1.35rem; line-height: 1; font-variant-numeric: tabular-nums;">${Math.max(1, Number(item.quantity) || 1)}x</span>
              <span style="color: var(--text-primary); font-weight: 700; font-size: 1.15rem; line-height: 1.2;">${escapeHtml(item.itemName || item.name || 'Item')}</span>
            </div>
            ${item.notes ? `<div style="font-size: var(--text-xs); color: #FF8960; font-weight: 500; font-style: italic; margin-left: 28px; margin-top: 4px; display: flex; align-items: center; gap: 4px;">
              <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 12px;">notes</span>
    ...
    if (order.status === 'confirmed') {
      actionBtnHtml = `
        <button class="btn btn-primary btn-block btn-sm action-btn" data-action="prepare" data-id="${orderId}" style="
          ...
          height: 38px;
        ">
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 18px;">restaurant</span>
          Start Cook
        </button>
      `;
    } else if (order.status === 'preparing') {
      actionBtnHtml = `
        <button class="btn action-btn" data-action="ready" data-id="${orderId}" style="
          ...
          transition: all var(--transition-fast);
        ">
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 18px;">check_circle</span>
          Food Ready
        </button>
      `;
    } else if (order.status === 'ready') {
      ...
      actionBtnHtml = `
        <button class="btn action-btn" data-action="${action}" data-id="${orderId}" style="
          ...
          transition: all var(--transition-fast);
        ">
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 18px;">done_all</span>
          ${label}
        </button>
      `;
    }
    ...
    card.innerHTML = `
      ...
      ${order.customerName ? `
        <div style="
          ...
          gap: 6px;
        ">
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 14px; color: var(--color-primary-on-surface);">person</span>
      ...
      <div style="margin-top: 4px; display: flex; gap: 8px;">
        ...
        " title="Cancel/Void Order">
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 18px;">delete</span>
        </button>
      </div>
    `;
