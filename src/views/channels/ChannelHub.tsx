    ...
    this.container.innerHTML = `
      <div style="flex:1;display:flex;flex-direction:column;height:100%;overflow:hidden;background:var(--bg-primary);">
        <div style="display:flex;align-items:center;gap:10px;padding:16px 24px;background:var(--glass-bg);backdrop-filter:blur(20px);border-bottom:1px solid var(--border-glass);z-index:10;">
          <span aria-hidden="true" class="material-symbols-rounded" style="color: var(--color-primary-on-surface);font-size:24px;">hub</span>
          ...
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:14px;">
            ${channelData.map(ch => `
              <div style="padding:20px;background:rgba(255,255,255,0.01);border:1px solid var(--border-glass);border-radius:16px;position:relative;overflow:hidden;">
                ${ch.status === 'coming' ? `<div style="position:absolute;top:10px;right:10px;font-size:0.55rem;padding:2px 8px;border-radius:6px;font-weight:700;color:var(--nextgenos-purple-on-surface);background:rgba(108,92,231,0.1);border:1px solid rgba(108,92,231,0.2);">Coming Soon</div>` : `<div style="position:absolute;top:12px;right:12px;width:8px;height:8px;border-radius:50%;background:var(--color-success);box-shadow:0 0 6px rgba(var(--color-success-rgb),0.5);"></div>`}
                <div style="width:44px;height:44px;border-radius:12px;background:rgba(${ch.status === 'active' ? '255,107,53' : '148,163,184'},0.08);display:flex;align-items:center;justify-content:center;margin-bottom:14px;">
                  <span aria-hidden="true" class="material-symbols-rounded" style="font-size:24px;color:${ch.color};">${ch.icon}</span>
                </div>
            ...
            <div style="display:flex;flex-direction:column;gap:8px;">
              ${recentOrders.length === 0 ? '<div style="text-align:center;padding:40px;color:var(--text-muted);font-size:var(--text-xs);">No orders yet</div>' :
                recentOrders.map(o => {
                  ...
                  const statusColor = o.status === 'completed' ? 'var(--color-success-on-surface)' : o.status === 'preparing' ? 'var(--color-warning-on-surface)' : o.status === 'ready' ? 'var(--color-info-on-surface)' : 'var(--text-muted)';
                  ...
                  return `
                    <div style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:rgba(255,255,255,0.01);border:1px solid var(--border-glass);border-radius:12px;">
                      <span aria-hidden="true" class="material-symbols-rounded" style="font-size:18px;color:${chCfg.color};">${chCfg.icon}</span>
