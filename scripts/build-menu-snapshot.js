...
/**
 * Read STORE_ORIGIN out of brand.ts rather than keeping a second copy here.
 *
 * A "keep in step with" comment is not a mechanism. This was still pointing at
 * the previous brand's domain after the rename, and so was public/robots.txt,
 * which nothing generated at all — a crawler was being handed a sitemap URL on
 * a host that had never existed. Both are derived now.
 */
...
const STORE_ORIGIN = process.env.NEXT_PUBLIC_STORE_ORIGIN || readBrandOrigin();
// Data key, not a display name: it maps to the store_id column in Supabase
// and must not change with a rebrand.
const STORE_ID = process.env.NEXT_PUBLIC_STORE_ID || 'trending-juice';
