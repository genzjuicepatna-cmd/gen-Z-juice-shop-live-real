...
test('cloud staff access requires an active database membership', () => {
  ...
  assert.deepEqual(
    requireCloudStaffAccess(
      ...
      { store_id: 'trending-juice', role: 'owner', staff_id: 7, is_active: true }
    ),
    { role: 'owner', staffId: 7, storeId: 'trending-juice' }
  );
});

test('user-controlled metadata can never grant staff or developer access', () => {
  assert.equal(
    appMetadataToStaffAccess({ user_metadata: { role: 'developer', store_id: 'trending-juice' } }),
    null
  );
  assert.throws(
...
test('staff metadata is only a hint and must match the active store', () => {
  ...
  assert.deepEqual(
    appMetadataToStaffAccess({ app_metadata: { role: 'manager', stores: ['trending-juice'], staff_id: 8 } }),
    { role: 'manager', staffId: 8, storeId: 'trending-juice' }
  );
});

test('inactive memberships and unknown roles are rejected', () => {
  assert.equal(membershipToStaffAccess({ store_id: 'trending-juice', role: 'owner', is_active: false }), null);
  assert.equal(membershipToStaffAccess({ store_id: 'trending-juice', role: 'admin', is_active: true }), null);
});

test('developer role is accepted as a valid staff role with full access', () => {
  // Developer membership is accepted
  assert.deepEqual(
    membershipToStaffAccess({ store_id: 'trending-juice', role: 'developer', is_active: true }),
    { role: 'developer', staffId: null, storeId: 'trending-juice' }
  );
  ...
  assert.deepEqual(
    requireCloudStaffAccess(
      { user_metadata: { role: 'customer' } },
      { store_id: 'trending-juice', role: 'developer', staff_id: 99, is_active: true }
    ),
    { role: 'developer', staffId: 99, storeId: 'trending-juice' }
  );
});
