...
test('the mobile bottom navigation stays pinned to the viewport while scrolling', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/#/self-order');
  await expect(page.getByRole('heading', { name: BRAND.name, level: 1 })).toBeVisible();

  const nav = page.getByRole('navigation', { name: 'Mobile customer navigation' });
  await expect(nav).toBeVisible();

  // The entry animation genuinely holds a transform for its 450ms, and during
  // that window `fixed` really does resolve against <main>. The bug was that
  // it never went away. Measure once the animation has finished.
  ...
  // transform, filter, backdrop-filter, perspective, will-change or paint
  // containment — not necessarily the viewport. `.public-main` used to keep an
  // identity transform after its entry animation (`slideUp ... forwards`), so
  // the bar was pinned to a 7,000px-tall <main> and you had to scroll to the
  // bottom of the page to reach it. This asserts the bar does not move.
  expect(afterScroll.y).toBeCloseTo(atTop.y, 0);
  expect(afterScroll.y + afterScroll.height).toBeLessThanOrEqual(844 + 1);

  const culprits = await page.evaluate(() => {
    const found = [];
    let el = document.querySelector('.customer-bottom-nav')?.parentElement;
  ...
  expect(culprits, 'an ancestor is creating a containing block for the fixed nav').toEqual([]);
});
