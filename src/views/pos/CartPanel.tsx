  ...
  render() {
    this.container.innerHTML = `
      ...
      <div class="cart-header" style="display: flex; justify-content: space-between; align-items: center;">
        <h2>
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 22px;">shopping_cart</span>
          Current Order
        ...
        <button class="btn btn-secondary btn-sm cart-mobile-back" id="cart-mobile-back" style="display: none; padding: 6px 12px; font-size: var(--text-xs); align-items: center; gap: 4px;">
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 16px;">arrow_back</span>
          Menu
        </button>
      </div>
      ...
      <div class="cart-order-type">
        <div class="order-type-selector" id="order-type-selector">
          <button class="order-type-btn ${this.orderType === 'takeaway' ? 'active' : ''}" data-type="takeaway">
            <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 16px;">shopping_bag</span>
            Takeaway
          </button>
          <button class="order-type-btn ${this.orderType === 'dinein' ? 'active' : ''}" data-type="dinein">
            <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 16px;">restaurant</span>
            Dine In
          </button>
          <button class="order-type-btn ${this.orderType === 'delivery' ? 'active' : ''}" data-type="delivery">
            <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 16px;">delivery_dining</span>
            Delivery
          </button>
        </div>
      </div>
    ...
    if (this.cart.length === 0) {
      return `
        <div class="cart-empty">
          <span aria-hidden="true" class="material-symbols-rounded">receipt_long</span>
            ...
            <div class="cart-item-actions">
              <div class="stepper">
                <button class="stepper-minus" data-index="${index}" aria-label="Decrease quantity">
                  <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 18px;">remove</span>
                </button>
                ...
                <button class="stepper-plus" data-index="${index}" aria-label="Increase quantity">
                  <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 18px;">add</span>
                </button>
              </div>
            </div>
    ...
    return `
      ...
      <div class="cart-actions">
        ...
        <button class="btn btn-ghost btn-block btn-sm" id="btn-clear-cart">
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 18px;">delete_sweep</span>
          Clear Order
        </button>
      </div>
    `;
