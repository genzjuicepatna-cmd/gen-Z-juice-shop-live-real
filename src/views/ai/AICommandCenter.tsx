...
const TIER_BADGES = {
  codebase: { label: '💻 Local AI', color: 'var(--text-muted)', bg: 'var(--text-muted)' },
      ...
      <div style="flex:1;display:flex;flex-direction:column;height:100%;overflow:hidden;background:var(--bg-primary);">
        ...
        <div style="display:flex;align-items:center;gap:12px;padding:16px 24px;background:var(--glass-bg);backdrop-filter:blur(20px);border-bottom:1px solid var(--border-glass);z-index:10;">
          <div style="width:40px;height:40px;border-radius:12px;background:rgba(108,92,231,0.1);border:1px solid rgba(108,92,231,0.25);display:flex;align-items:center;justify-content:center;">
            <span aria-hidden="true" class="material-symbols-rounded" style="font-size:22px;color:var(--nextgenos-purple-on-surface);filter:drop-shadow(0 0 6px rgba(108,92,231,0.4));">smart_toy</span>
          </div>
          <div>
          ...
          <div style="margin-left:auto;display:flex;gap:6px;">
            ...
            ${aiService.isLightningAvailable ? '<span style="padding:3px 8px;border-radius:12px;background:rgba(var(--nextgenos-purple-rgb),0.08);border:1px solid rgba(var(--nextgenos-purple-rgb),0.2);font-size:0.6rem;color:var(--nextgenos-purple-on-surface);font-weight:600;">🧠 Lightning</span>' : ''}
            <span style="padding:3px 8px;border-radius:12px;background:var(--text-muted);border:1px solid var(--text-muted);font-size:0.6rem;color:var(--text-muted);font-weight:600;">💻 Local</span>
          </div>
        </div>
        ...
        <!-- Input Bar -->
        <div style="padding:16px 24px;background:var(--bg-surface);border-top:1px solid var(--border-color);">
          <div style="display:flex;gap:10px;max-width:800px;margin:0 auto;">
            <input type="text" id="ai-input" placeholder="Ask anything about your business..." style="flex:1;padding:12px 16px;background:var(--bg-input);border:1px solid var(--border-color);border-radius:12px;color:var(--text-primary);font-size:var(--text-sm);font-family:var(--font-sans);outline:none;transition:border-color 0.2s;">
            <button id="ai-send" aria-label="Send message" title="Send message" style="width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,var(--nextgenos-purple-on-surface),var(--nextgenos-purple-on-surface));border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:transform 0.15s,box-shadow 0.2s;box-shadow:0 4px 15px rgba(108,92,231,0.3);flex-shrink:0;">
              <span aria-hidden="true" class="material-symbols-rounded" style="font-size:20px;color:white;">send</span>
            </button>
          </div>
        </div>
      </div>
  ...
  addAIMessage(text, suggestions = [], tier = 'codebase') {
    ...
    const chipsHTML = suggestions.length > 0 ? `
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:12px;">
        ${suggestions.map(s => `<button class="ai-chip" style="padding:6px 14px;border-radius:20px;background:rgba(108,92,231,0.08);border:1px solid rgba(108,92,231,0.2);color:var(--nextgenos-purple-on-surface);font-size:0.75rem;font-weight:600;cursor:pointer;transition:all 0.2s;font-family:var(--font-sans);">${this.escapeHtml(s)}</button>`).join('')}
      </div>
    ` : '';
        ...
        <div style="padding:16px 18px;background:rgba(255,255,255,0.02);border:1px solid var(--border-glass);border-radius:4px 16px 16px 16px;color:var(--text-primary);font-size:var(--text-sm);line-height:1.6;backdrop-filter:blur(8px);">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
            <div style="display:flex;align-items:center;gap:5px;font-size:0.6rem;color:var(--nextgenos-purple-on-surface);font-weight:600;letter-spacing:0.06em;text-transform:uppercase;">
              <span aria-hidden="true" class="material-symbols-rounded" style="font-size:13px;">smart_toy</span> AI Assistant
            </div>
  ...
  addTypingIndicator() {
    ...
    div.innerHTML = `
      <div style="padding:12px 18px;background:rgba(255,255,255,0.02);border:1px solid var(--border-glass);border-radius:4px 16px 16px 16px;display:flex;gap:5px;align-items:center;">
        <span style="width:7px;height:7px;border-radius:50%;background:var(--nextgenos-purple-on-surface);animation:bounce 1.4s infinite;animation-delay:0s;opacity:0.7;"></span>
        <span style="width:7px;height:7px;border-radius:50%;background:var(--nextgenos-purple-on-surface);animation:bounce 1.4s infinite;animation-delay:0.2s;opacity:0.7;"></span>
        <span style="width:7px;height:7px;border-radius:50%;background:var(--nextgenos-purple-on-surface);animation:bounce 1.4s infinite;animation-delay:0.4s;opacity:0.7;"></span>
      </div>
    `;
    container.appendChild(div);
    this.scrollToBottom();
