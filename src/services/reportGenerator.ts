...
async function buildReportCsv(title, label, orders) {
  const restaurantName = await getSetting('restaurantName') || BRAND.name;
...
export async function generateDailyReport(dateStr) {
  ...
  return buildReportCsv(`${BRAND.name} POS - DAILY BUSINESS REPORT`, dateStr, orders);
}
...
export async function generateWeeklyReport(endDateStr) {
  ...
  start.setHours(0, 0, 0, 0);
  const orders = await getOrdersForRange(start.toISOString(), end.toISOString());
  return buildReportCsv(`${BRAND.name} POS - WEEKLY BUSINESS REPORT`, `${start.toLocaleDateString('en-IN')} to ${end.toLocaleDateString('en-IN')}`, orders);
}
...
export async function generateMonthlyReport(month, year) {
  ...
  return buildReportCsv(`${BRAND.name} POS - MONTHLY BUSINESS REPORT`, label, orders);
}
