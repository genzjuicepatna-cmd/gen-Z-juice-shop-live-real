...
function StaffManager() {
  ...
  const ROLES = {
    developer: { label: 'Developer', color: 'var(--nextgenos-purple-on-surface)', icon: 'terminal' },
    ...
    kitchen: { label: 'Kitchen', color: 'var(--color-warning-on-surface)', icon: 'restaurant' },
    ...
    <div style={{ padding: '28px 24px', display: 'flex', flexDirection: 'column', gap: '28px', maxWidth: '1100px', margin: '0 auto', width: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-lg)', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '24px', color: 'var(--color-primary)', filter: 'drop-shadow(0 0 8px var(--color-primary))' }}>groups</span>
            ...
            <div key={s.id} className="premium-card" style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ marginTop: '12px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <select
                    className="input"
                    aria-label={`Role for ${s.name || 'this staff member'}`}
                    value={s.role}
                  ...
                  >
                    <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '18px' }}>{s.isActive ? 'person_off' : 'person'}</span>
                  </button>
                  ...
                  {isDeletable && (
                    ...
                    >
                      <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '18px' }}>delete</span>
                    </button>
                  )}
                </div>
              </div>
      ...
      <div className="card" style={{ padding: '24px', background: 'var(--glass-bg)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', overflowX: 'auto' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '18px', letterSpacing: '-0.01em', display: 'flex', alignItems: 'center', gap: '6px' }}>
          <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '20px', color: 'var(--color-info)' }}>security</span>
          Role Access Matrix
        </div>
  ...
  return (
    <div className="main-area" style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'radial-gradient(circle at bottom left, rgba(var(--color-primary-rgb), 0.04) 0%, transparent 60%)' }}>
      ...
      <div className="header-bar" style={{
        ...
        <div className="tab-container scrollbar-none" style={{
          ...
          {[
            ...
            // "Menu CRUD" and "Order Log" were developer names on a screen
            // the shop owner uses, not the developer.
            { id: 'menu', label: 'Menu', icon: 'edit_document' },
            { id: 'orders', label: 'Past orders', icon: 'history' },
            ...
            return (
              <button
                ...
                style={{
                  display: 'inline-flex',
                  alignItems: 'center',
                  // White on --gradient-primary is 2.1:1 at the mango end.
                  // axe never caught it because it cannot compute contrast
                  // against a gradient, so it skips the node entirely.
                  background: isActive ? 'var(--color-primary)' : 'var(--bg-surface)',
                  color: isActive ? 'var(--color-primary-ink)' : 'var(--text-primary)',
              ...
              >
                <span className="material-symbols-rounded" aria-hidden="true" style={{ fontSize: '18px', marginRight: '6px', color: isActive ? 'var(--color-primary-ink)' : 'var(--text-secondary)' }}>{tab.icon}</span>
                {tab.label}
              </button>
            );
          })}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
          ...
          >
            <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '16px' }}>lock</span>
            Lock Terminal
          </button>
        </div>
      </div>
      ...
      {/* A scrollable region needs to be focusable, or a keyboard user cannot
          scroll it at all — there is nothing tabbable inside to carry them
