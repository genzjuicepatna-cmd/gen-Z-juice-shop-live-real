...
import { itemArt, FALLBACK_CATEGORY_ART } from '../../content/categoryArt';
export class MenuGrid {
    ...
    this.container.innerHTML = `
      ...
      <div class="search-container">
        <div class="search-wrapper">
          <span aria-hidden="true" class="material-symbols-rounded search-icon">search</span>
          <input 
            type="text" 
            class="input" 
            id="menu-search" 
    ...
    if (this.filteredItems.length === 0) {
      return `
        <div class="empty-state" style="padding-top: 60px;">
          <span aria-hidden="true" class="material-symbols-rounded">restaurant_menu</span>
  ...
  getMenuItemImage(item) {
    ...
    // Per item, not per category: a cashier scanning the grid needs the eight
    // cold-pressed juices to look different from each other. The category tile
    // made every card in a section identical.
  ...
  renderMenuItem(item) {
    ...
    const resolvedImage = safeImageUrl(this.getMenuItemImage(item), FALLBACK_CATEGORY_ART);
