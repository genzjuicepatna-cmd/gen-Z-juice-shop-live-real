...
export function AnalyticsView() {
  ...
  const loadAnalyticsData = async () => {
    ...
    try {
      ...
      const storeId = localStorage.getItem('store_id') || STORE_ID;
      const now = new Date();
      let startDate = null;
  ...
  if (error) {
    return (
      <div style={{ maxWidth: '500px', margin: '80px auto', padding: '32px', textAlign: 'center' }} className="card">
        <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '48px', color: 'var(--color-danger)', marginBottom: '12px' }}>cloud_off</span>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
        <div>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-lg)', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.02em', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '24px', color: 'var(--color-primary)' }}>analytics</span>
      ...
      <div id="analytics-dashboard-content" style={{ display: 'flex', flexDirection: 'column', gap: '24px', transition: 'opacity 0.2s', opacity: isLoading ? 0.35 : 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
          <div className="stats-card-v3 revenue">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span className="stats-card-label" style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-secondary)' }}>CLOUD REVENUE</span>
              <span aria-hidden="true" className="material-symbols-rounded" style={{ color: 'var(--color-success-on-surface)', fontSize: '18px' }}>payments</span>
            </div>
          ...
          <div className="stats-card-v3 orders">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span className="stats-card-label" style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-secondary)' }}>PAID TRANSACTIONS</span>
              <span aria-hidden="true" className="material-symbols-rounded" style={{ color: 'var(--color-primary)', fontSize: '18px' }}>receipt_long</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '12px' }}>
          ...
          <div className="stats-card-v3 avgval">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span className="stats-card-label" style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-secondary)' }}>AVERAGE TICKET</span>
              <span aria-hidden="true" className="material-symbols-rounded" style={{ color: 'var(--color-info)', fontSize: '18px' }}>analytics</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '12px' }}>
          ...
          <div className="stats-card-v3 loyalty">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span className="stats-card-label" style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-secondary)' }}>NEW SIGNUPS</span>
              <span aria-hidden="true" className="material-symbols-rounded" style={{ color: 'var(--color-warning)', fontSize: '18px' }}>stars</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '12px' }}>
        ...
        <div className="card" style={{
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', fontWeight: 700 }}>Peak Revenue: {formatCurrency(trendData.maxRevenue)}</span>
          </div>
          ...
          {/* Labels X */}
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: `0 ${trendData.paddingX}px`, marginTop: '12px', fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', fontWeight: 700 }}>
            {stats.dailyTrend.map((t, i) => {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px' }}>
                  ...
                  return (
                    <div key={t} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid var(--border-glass)', fontSize: 'var(--text-xs)', fontWeight: 700 }}>
                      <span style={{ color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span aria-hidden="true" className="material-symbols-rounded" style={{ fontSize: '18px', color: 'var(--color-primary)' }}>{icon}</span>
                        {name}
                      </span>
          ...
          <div className="card" style={{ padding: '24px', display: 'flex', flexDirection: 'column', height: '430px', background: 'linear-gradient(135deg, rgba(var(--color-success-rgb), 0.02) 0%, var(--glass-bg) 100%)', border: '1px solid var(--border-color)', borderLeft: '4px solid var(--color-success)', boxShadow: 'var(--shadow-sm)', borderRadius: 'var(--radius-md)' }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 'var(--text-xs)', padding: '2px 8px', background: 'rgba(var(--color-success-rgb), 0.08)', border: '1px solid rgba(var(--color-success-rgb),0.2)', color: 'var(--color-success-on-surface)', borderRadius: '4px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Sync Logs</span>
            </h3>
                      ...
                      <div className="audit-item-v2" onClick={() => {
                        ...
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                            <span style={{ fontSize: 'var(--text-xs)', fontWeight: 800, padding: '2px 6px', background: actionBadgeColor, border: `1px solid ${actionBadgeBorder}`, color: actionColor, borderRadius: '4px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.02em' }}>
                              {aud.action}
                            </span>
                            <span style={{ color: 'var(--text-primary)', fontWeight: 700 }}>on {aud.target_table}</span>
                            <span style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-xs)' }}>({aud.target_id})</span>
                          </div>
                        </div>
                        <div style={{ color: 'var(--text-secondary)', fontWeight: 700, textAlign: 'right', fontSize: 'var(--text-xs)', flexShrink: 0 }}>
                          <div>{timeStr}</div>
                          <div style={{ fontSize: 'var(--text-xs)', fontWeight: 500, opacity: 0.8, marginTop: '2px' }}>{dateStr}</div>
                        </div>
                      </div>
