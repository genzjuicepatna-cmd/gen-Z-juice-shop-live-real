    ...
    <div className="store-success-shell">
      <div className="store-success-panel customer-tracking-panel animate-fade-in">
        <section className="customer-tracking-hero">
          ...
          <p>{liveOrder.type === 'delivery' ? 'We will keep this page updated from the counter to your doorstep.' : liveOrder.type === 'takeaway' ? 'Keep this token ready for pickup.' : 'Your table order has been sent to the counter.'}</p>
        ...
        {tracking.canReview && (
          <section className="customer-review-card">
            <div className="customer-section-title"><div><small>Review</small><h2>{reviewSaved ? 'Thanks for rating us' : 'How was it?'}</h2></div><Icon>reviews</Icon></div>
            {!reviewSaved ? (
              <>
                ...
                <textarea value={review} onChange={(e) => setReview(e.target.value)} placeholder="Optional note for the shop" rows="3" />
