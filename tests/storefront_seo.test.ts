...
import { STORE_ORIGIN } from '../src/content/brand';
...
test('storefront wording is owner-editable rather than hardcoded', () => {
  ...
  assert.doesNotMatch(source, /Cold-pressed juice, thick smoothies and shakes, blended the second/);
...
test('featured dishes survive a rename', () => {
  ...
  assert.doesNotMatch(source, /const names = \['Classic Orange'/);
...
test('the boot splash is a styled opaque overlay', () => {
  ...
  // The splash has to be opaque and has to land on the same colour the
  // storefront paints, or the SSG -> SPA handoff flashes. It used to assert
  // the literal #040406, which pinned the splash to a dark palette the
  // storefront no longer uses; what matters is the cream base, not the hex
  // that happened to be there.
  const splash = css.match(/#loading-screen \{[\s\S]*?\n\}/)?.[0] ?? '';
  assert.match(splash, /background:[\s\S]*?#100B16;/, 'splash must end on the storefront ground');
  assert.match(splash, /color: #FFF7F2;/, 'splash text must be the storefront ink');

  // Same base colour on both sides of the handoff.
  assert.match(read('src/styles/storefront.css'), /--store-soft: #100B16;/);
});

test('the boot splash is dropped from the accessibility tree as it fades', () => {
  // `.hide` only animates opacity and the node lives on for another 500ms, so
  // without this it stays focusable and axe-visible on top of the running app.
  const main = read('src/main.ts');
  const hide = main.match(/hideLoadingScreen\(\) \{[\s\S]*?\n  \}/)?.[0] ?? '';
  assert.match(hide, /aria-hidden/);
  assert.match(hide, /inert/);
});
