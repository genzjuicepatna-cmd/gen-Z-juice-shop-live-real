...
class AIService {
  ...
  async queryGroq(query, intent, queryEmbedding = null) {
    ...
    const systemPrompt = `You are an AI assistant for "${context.restaurantName || BRAND.name}" restaurant.
...
- Payment methods breakdown: ${context.paymentBreakdown}`;
  ...
  async gatherBusinessContext() {
    ...
    const restaurantName = localStorage.getItem('app_restaurant_name') || await getSetting('restaurantName') || BRAND.name;
