      ...
      case 'quickstart':
        return (
          <div className="help-doc-section">
            ...
            <p>Welcome to <strong>${BRAND.name} Operating System</strong>. Follow this quick start setup to begin operations.</p>
            ...
            <div className="help-step-card">
              ...
              <div>
                ...
                <p style={{ margin: '4px 0 0 0', fontSize: '13px' }}>Modify quantities inside the stepper (`+` / `-`). Long-press or tap an item in the cart to append preparation notes (e.g., "Less sugar", "No ice").</p>
              </div>
            </div>
      ...
      case 'operations':
        return (
          <div className="help-doc-section">
            ...
            <div style={{ display: 'flex', gap: '8px', margin: '16px 0', fontSize: 'var(--text-sm)', justifyContent: 'space-between', flexWrap: 'wrap' }}>
              <span style={{ background: 'rgba(var(--color-danger-rgb), 0.1)', border: '1px solid rgba(var(--color-danger-rgb),0.2)', padding: '6px 10px', borderRadius: '4px', color: 'var(--color-danger-on-surface)', fontWeight: 700 }}>1. PENDING (Unpaid/Self)</span>
              <span style={{ background: 'rgba(var(--color-warning-rgb), 0.1)', border: '1px solid rgba(var(--color-warning-rgb),0.2)', padding: '6px 10px', borderRadius: '4px', color: 'var(--color-warning-on-surface)', fontWeight: 700 }}>2. PREPARING</span>
          ...
          <div className="help-doc-section">
            ...
            <div className="help-step-card">
              ...
              <div>
                ...
                <p style={{ margin: '4px 0 0 0', fontSize: '13px' }}>Click the <strong>"Printer off"</strong> button in the app header, or navigate to <strong>Admin {'→'} Settings {'→'} Printer Setup</strong>.</p>
              </div>
            </div>
        ...
        return (
          <div className="help-doc-section">
            ...
            <div className="help-tip-box">
              ...
              <br />• <span style={{ color: 'var(--color-warning-on-surface)', fontWeight: 700 }}>Orange (Syncing):</span> Pushing offline logs.
              <br />• <span style={{ color: 'var(--color-danger-on-surface)', fontWeight: 700 }}>Red (Offline):</span> Running on local storage database.
            </div>
          </div>
        );
      ...
      <div className="help-sidebar">
        <div style={{ marginBottom: '16px', paddingLeft: '8px' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-sm)', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.01em', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span aria-hidden="true" className="material-symbols-rounded txt-brand" style={{ fontSize: '22px' }}>help_center</span>
            Help Center
          </div>
          <div style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)', marginTop: '4px' }}>Documentation & Guides</div>
        </div>

        <button className={`help-nav-btn ${activeSection === 'quickstart' ? 'active' : ''}`} onClick={() => handleNavClick('quickstart')}>
          <span aria-hidden="true" className="material-symbols-rounded">rocket_launch</span>
          Quick Start Guide
        </button>
        <button className={`help-nav-btn ${activeSection === 'operations' ? 'active' : ''}`} onClick={() => handleNavClick('operations')}>
          <span aria-hidden="true" className="material-symbols-rounded">restaurant</span>
          Operations & Flow
        </button>
        <button className={`help-nav-btn ${activeSection === 'hardware' ? 'active' : ''}`} onClick={() => handleNavClick('hardware')}>
          <span aria-hidden="true" className="material-symbols-rounded">print</span>
          Hardware Setup
        </button>
        <button className={`help-nav-btn ${activeSection === 'cloud' ? 'active' : ''}`} onClick={() => handleNavClick('cloud')}>
          <span aria-hidden="true" className="material-symbols-rounded">cloud_sync</span>
          Cloud Sync & Offline
        </button>
        <button className={`help-nav-btn ${activeSection === 'ai' ? 'active' : ''}`} onClick={() => handleNavClick('ai')}>
          <span aria-hidden="true" className="material-symbols-rounded">smart_toy</span>
          AI Command Center
        </button>
        <button className={`help-nav-btn ${activeSection === 'faq' ? 'active' : ''}`} onClick={() => handleNavClick('faq')}>
          <span aria-hidden="true" className="material-symbols-rounded">quiz</span>
          FAQs
        </button>

        <div style={{ marginTop: 'auto', padding: '12px', background: 'rgba(var(--color-primary-rgb), 0.05)', border: '1px solid rgba(var(--color-primary-rgb), 0.1)', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', textAlign: 'center' }}>
