...
test.describe('pre-rendered storefront', () => {
  ...
  test('the menu page lists every dish with a price', async ({ page }) => {
    ...
    // Smoke check that the page rendered its own hero rather than a shell.
    // The wording lives in src/app/menu/page.tsx; update both together.
    await expect(page.getByRole('heading', { level: 1 })).toContainText('Everything we make');
