...
class App {
  ...
  async onLoginSuccess(staff) {
    // The login screen is a React root mounted on #app, and this runs from
    // inside one of its event handlers. Everything below takes over #app
    // imperatively with innerHTML, so React has to let go of the container
    // first — otherwise it reconciles against markup it did not create,
    // throws `removeChild ... not a child of this node`, and wipes the shell
    // we just built. That left #app-sidebar null and surfaced as "Failed to
    // load staff console", which is what the Retry button was papering over:
    // the retry only worked because by then the root was gone.
    //
    // Yielding a macrotask lets React finish its current commit before the
    // unmount, which also avoids the synchronous-unmount-while-rendering
    // warning.
    if (this.loginScreen) {
      const loginScreen = this.loginScreen;
      this.loginScreen = null;
    ...
    if (staff && staff.role === 'customer') {
      ...
      router.navigate('#/self-order');
      this.initialized = true;
      console.log(`🧃 ${BRAND.name} — Customer ${staff.name} logged in successfully.`);
      return;
    }
    ...
    try {
      ...
      this.initialized = true;
      console.log(`🧃 ${BRAND.name} OS — Logged in as ${staff.name} (${staff.role})`);
    } catch (error) {
      ...
      if (mainEl) {
        mainEl.innerHTML = `
          <div class="empty-state" style="height: 80vh; text-align: center; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 16px;">
            <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 48px; color: var(--color-error, #EF4444);">error</span>
        ...
        <div class="main-area">
          ...
          <header class="app-header">
            <button class="btn-icon sidebar-mobile-toggle" id="sidebar-toggle" title="Toggle Menu">
              <span aria-hidden="true" class="material-symbols-rounded">menu</span>
            </button>
            <a href="#/pos" class="logo" id="app-logo">
              <img src="/assets/store-logo.svg" class="logo-img" alt="Logo" style="width:28px;height:28px;border-radius:6px;object-fit:contain;margin-right:8px;border:1px solid var(--border-active);box-shadow:var(--shadow-glow-active);" />
              <span style="font-weight: 800; font-family: var(--font-display); letter-spacing: -0.04em;">${BRAND.name}</span>
            </a>
            <button class="btn-icon" id="btn-theme-toggle" title="Toggle Theme" style="margin-left: 4px; color: var(--text-muted);">
              <span aria-hidden="true" class="material-symbols-rounded" id="theme-toggle-icon" style="font-size: 18px;">${App.getResolvedTheme() === 'dark' ? 'light_mode' : 'dark_mode'}</span>
            </button>
            ...
            <div class="header-actions">
              <div class="header-printer-status" id="header-staff-display" title="Logged in staff" style="gap:4px;">
                <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 16px;">person</span>
              ...
              <button class="header-printer-status" id="sync-status-btn" title="Cloud Sync Status">
                <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 16px;">cloud_sync</span>
              ...
              <button class="header-printer-status" id="printer-status-btn" title="Printer Status">
                <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 16px;">print</span>
                <span id="printer-status-text">Printer off</span>
              ...
              <button class="btn-icon" id="btn-install-app" title="Install App" style="display:none;">
                <span aria-hidden="true" class="material-symbols-rounded">install_mobile</span>
              </button>
              <button class="btn-icon" id="btn-logout" title="Logout" style="color:var(--text-muted);">
                <span aria-hidden="true" class="material-symbols-rounded">logout</span>
              </button>
            </div>
          </header>
  ...
  setupPWA() {
    ...
    window.addEventListener('appinstalled', () => {
      showToast(`${BRAND.name} OS installed successfully!`, 'success');
      this.deferredInstallPrompt = null;
      const installBtn = document.getElementById('btn-install-app');
  ...
  showUpdateBanner(onConfirm) {
    ...
    banner.innerHTML = `
      <div class="pwa-update-content">
        <span aria-hidden="true" class="material-symbols-rounded pwa-update-icon">system_update</span>
  ...
  async setupPrinter() {
    ...
    printerService.onStatusChange = (isConnected) => {
      ...
      if (text) {
        // "Not Connected" sat next to "Cloud Active" with no subject, so
        // the header read as two contradictory verdicts on the same thing.
        text.textContent = isConnected ? 'Printer ready' : 'Printer off';
      }
    };
  }
  ...
  hideLoadingScreen() {
    ...
    if (loadingScreen) {
      // `.hide` only fades to opacity 0, and the node lives on for another
      // 500ms. During that window it is still in the accessibility tree and
      // still focusable, sitting on top of the app: axe reports its light
      // brand text against the now-visible page beneath as a contrast
      // failure, and a keyboard user can tab into an invisible overlay.
      // Dropping it from the tree at the moment it stops being meaningful
      // costs nothing and leaves the fade intact.
  ...
  injectSandboxWidget() {
    ...
    widget.innerHTML = `
      <div class="aether-sandbox-trigger">
        <span aria-hidden="true" class="material-symbols-rounded">admin_panel_settings</span>
      ...
      <div class="aether-sandbox-panel">
        ...
        <div class="aether-sandbox-body">
          <button class="sandbox-panel-btn" id="sandbox-go-kiosk">
            <span aria-hidden="true" class="material-symbols-rounded" style="color:var(--color-primary);">shopping_cart</span>
            <div>
          ...
          <button class="sandbox-panel-btn" id="sandbox-go-pos">
            <span aria-hidden="true" class="material-symbols-rounded" style="color:var(--nextgenos-purple);">point_of_sale</span>
            <div>
