# Trending Juice production launch guide
...
`supabase/migrations/` holds **10 files, roughly 1,600 lines**, and they are
ordered — the timestamp prefix is the order. `db push` applies the ones the
project has not seen and records them in `supabase_migrations.schema_migrations`,
so re-running it is safe and does nothing on an up-to-date project.

**Prefer `db push` over pasting into the SQL editor.** Pasting works — the
statements are written `if not exists` — but nothing gets recorded as applied.
The first later `db push` then tries to replay all ten against a database that
already has them, and the ones that are not idempotent will fail.

The base schema needs two extensions, `pgcrypto` and `vector`. Both ship with
Supabase; `db push` creates them.

Check what a project already has before pushing:

```powershell
npx supabase migration list
```

`npm run db:validate` parses all ten locally and confirms the ordering without
touching a database. It is part of `npm run launch:verify`.

`STORE_ID` is set to `trending-juice`. It is the tenant key — the `store_id` column
default on tables. `ORDER_PREFIX` must match `BRAND.orderPrefix`
in `src/content/brand.ts`, or online orders get numbered differently from the
ones rung up at the counter.
...
```powershell
npx supabase secrets set STORE_ID=trending-juice GST_PERCENT=5 DELIVERY_FEE=0 ORDER_PREFIX=TRJ
...
Delete the local admin password from the workstation after provisioning or move it into an approved secret manager. Confirm the owner has an active `staff_memberships` row for `trending-juice`.

Once a developer account exists, further owner and staff logins are created
from **Admin → Staff → Create login**, which calls the `invite-staff` action on
`staff-admin`. That action is developer-only for the owner and developer roles,
refuses an email that already has an auth account, and never echoes a password
back to the browser.
...
```powershell
$env:STORE_KEYSTORE='C:\secure\trending-juice-release.jks'
$env:STORE_KEYSTORE_PASSWORD='FROM_SECRET_MANAGER'
$env:STORE_KEY_ALIAS='trending-juice'
$env:STORE_KEY_PASSWORD='FROM_SECRET_MANAGER'
.\build-apk.ps1
```
