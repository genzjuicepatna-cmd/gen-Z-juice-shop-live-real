...
$resolvedKeystore = (Resolve-Path -LiteralPath $env:STORE_KEYSTORE).Path
if (-not (Test-Path -LiteralPath $resolvedKeystore -PathType Leaf)) {
    throw 'STORE_KEYSTORE must point to an existing keystore file.'
}
$env:STORE_KEYSTORE = $resolvedKeystore
...
function Build-Portal([string]$PortalName, [string]$AppId, [string]$AppName, [string]$ArtifactName) {
    ...
    $gradleContent = $originalGradle -replace 'applicationId\s+"com\.trendingjuice\.pos"', "applicationId `"$AppId`""
...
try {
    if ($Portal -in @('pos', 'all')) {
        Build-Portal 'pos' 'com.trendingjuice.pos' 'Trending Juice POS' 'TrendingJuicePOS-release.apk'
    }
    if ($Portal -in @('customer', 'all')) {
        Build-Portal 'customer' 'com.trendingjuice.customer' 'Trending Juice' 'TrendingJuiceCustomer-release.apk'
    }
    if ($Upload) {
