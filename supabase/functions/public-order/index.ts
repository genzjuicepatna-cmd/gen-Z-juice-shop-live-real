...
const STORE_ID = Deno.env.get("STORE_ID") || "trending-juice";
const GST_PERCENT = Number(Deno.env.get("GST_PERCENT") || "5");
const DELIVERY_FEE = Number(Deno.env.get("DELIVERY_FEE") || "0");
// Must match BRAND.orderPrefix in src/content/brand.ts. Edge functions are
// Deno and cannot import from src/, so this is a second copy — but an
// unset secret should not silently number online orders differently from
// the ones rung up at the counter. It defaulted to the previous brand's
// "TT" while the POS already produced "TRJ".
const ORDER_PREFIX = Deno.env.get("ORDER_PREFIX") || "TRJ";
...
const RATE_LIMIT_SALT = Deno.env.get("PUBLIC_ORDER_RATE_LIMIT_SALT") || "store-public-order";
