...
It is designed for **small-to-medium food businesses in India** (currently Trending Juice, a juice corner in Kumhrar, Patna), but the architecture uses a `store_id` tenant model making it **multi-tenant ready** for a worldwide SaaS launch.
...
| RLS policies scoped by store_id | ✅ Done | Scoped to 'trending-juice' |
