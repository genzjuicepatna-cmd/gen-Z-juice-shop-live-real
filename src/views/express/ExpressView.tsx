...
import { db, createOrder, getOrders, updateOrderStatus, getNextOrderNumber, getSetting, getCategories, getAllItems } from '../../db/database';
...
export class ExpressView {
  ...
  async fetchKitchenOrders(fromCloud) {
    ...
    try {
      ...
      if (currentConfirmedCount > previousConfirmedCount) {
        const newestOrder = nextActiveOrders.filter(o => o.status === 'confirmed').pop();
  ...
  render() {
    ...
    const mobileCartHtml = this.cart.length > 0 ? `
      <div class="express-mobile-cart-bar" id="express-mobile-cart-bar">
        ...
        <div class="mobile-cart-action">
          ...
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size: 16px;">arrow_forward_ios</span>
        </div>
      </div>
    ` : '';
    ...
    this.container.innerHTML = `
        ...
        <div class="express-mode-bar">
          <div class="express-mode-switch" role="tablist" aria-label="Express panel mode">
            <button class="mode-btn ${this.activeTab === 'pos' ? 'active' : ''}" id="tab-btn-pos"
              ...
              <span aria-hidden="true" class="material-symbols-rounded">point_of_sale</span>
            ...
            <button class="mode-btn ${this.activeTab === 'kitchen' ? 'active' : ''}" id="tab-btn-kitchen"
              ...
              <span aria-hidden="true" class="material-symbols-rounded">restaurant</span>
          ...
          <div class="express-panel pos-panel">
            ...
            <div class="panel-header">
              <div class="compact-search">
                <span aria-hidden="true" class="material-symbols-rounded search-glass">search</span>
            ...
            <div class="express-pos-body">
              <!-- Grid of Products -->
              <!-- Focusable: the grid scrolls, and with the real 53-item catalogue it
                   overflows on every screen. Without a tab stop a keyboard user
                   can reach the tiles but never scroll to the ones below the fold. -->
              <div class="express-products-grid scrollbar-none" id="express-products-grid"
                   tabindex="0" role="group" aria-label="Menu items">
                ${this.renderProducts()}
              </div>
            ...
            <div class="express-cart-section" id="express-cart-section">
              <div class="express-cart-header">
                ...
                <button class="close-cart-btn" id="express-close-cart" type="button">
                  <span aria-hidden="true" class="material-symbols-rounded">arrow_back</span>
                  Menu
                </button>
                ...
                <div class="meta-row">
                  ...
                  <div class="express-type-selector">
                    <button class="type-btn ${this.orderType === 'takeaway' ? 'active' : ''}" data-type="takeaway">
                      <span aria-hidden="true" class="material-symbols-rounded">shopping_bag</span> Takeaway
                    </button>
                    <button class="type-btn ${this.orderType === 'dinein' ? 'active' : ''}" data-type="dinein">
                      <span aria-hidden="true" class="material-symbols-rounded">restaurant</span> Dine-In
                    </button>
                    <button class="type-btn ${this.orderType === 'delivery' ? 'active' : ''}" data-type="delivery">
                      <span aria-hidden="true" class="material-symbols-rounded">delivery_dining</span> Delivery
                    </button>
                  </div>
      ...
      <style>
        ...
        .express-pos-body {
          display: grid;
          /* 290px could not hold what was inside it. The order-type selector
             carried min-width: 240px for three labelled buttons, so Delivery
             was clipped by the right edge, and the checkout row pushed UPI QR
             off with it. Widened to fit the contents rather than shrinking the
             contents to fit an arbitrary column. */
        ...
        .express-type-selector {
          ...
          border-radius: var(--radius-full);
          padding: 3px;
          /* No min-width: it is the widest thing in the cart column, so a floor
             here is what overflowed the panel. It fills the row and its buttons
             share the space. */
          flex: 1 1 100%;
          min-width: 0;
        }
        ...
        .total-lbl {
          ...
          /* Wrapped to three lines at 290px — "TOTAL", "(0", "ITEMS)". */
          white-space: nowrap;
        }
        ...
        .btn-cash {
          ...
          /* Ink, not white: white on the lime end measures 1.66:1. */
        ...
        .btn-upi {
          ...
          /* This ramp is dark enough that white is the legible choice. */
          color: #FFFFFF;
  ...
  getMenuItemImage(item) {
    ...
    // Per item, not per category: every drink in a section rendered the same
    // tile, so the grid gave the operator nothing to aim at.
    const cat = this.categories.find(c => c.id === item.categoryId);
    return itemArt(item.name, cat ? cat.name : '');
  }
    ...
    return `
      ...
      <div class="checkout-buttons-group">
        <button class="quick-pay-btn btn-cash" id="express-pay-cash" ${this.cart.length === 0 ? 'disabled' : ''}>
          <span aria-hidden="true" class="material-symbols-rounded">payments</span>
          Cash
        </button>
        <button class="quick-pay-btn btn-upi" id="express-pay-upi" ${this.cart.length === 0 ? 'disabled' : ''}>
          <span aria-hidden="true" class="material-symbols-rounded">qr_code_2</span>
          UPI QR
        </button>
      </div>
    `;
    ...
    if (filtered.length === 0) {
      kdsFeed.innerHTML = `
        <div class="kds-empty-feed">
          <span aria-hidden="true" class="material-symbols-rounded" style="font-size:32px;">inbox</span>
    ...
    kdsFeed.innerHTML = filtered.map(order => {
      ...
      return `
        <div class="express-order-card status-${safeStatus}" id="kds-card-${orderId}">
          ...
          ${order.notes ? `<div style="font-size:10px; color: var(--color-warning-on-surface); font-style:italic;">Notes: ${escapeHtml(order.notes)}</div>` : ''}
  ...
  async printReceipt(order) {
    ...
    try {
      const settings = {
        restaurantName: await getSetting('restaurantName') || BRAND.name,
        restaurantTagline: await getSetting('restaurantTagline') || BRAND.tagline,
